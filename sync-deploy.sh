...
patch_merge_file() {
  ...
  # stage3 = v2+subs verbatim: right panel is exactly what source tag 2 looks like.
  # stage1 = v1+subs: diff(stage1, stage3) = exact v1→v2 lines → precise highlights.
  # Keys that target customised (e.g. replicas: 3 vs source 1) appear in the right
  # panel with the source value and no highlight — signalling "unchanged in source,
  # your override stands."  Only lines the source actually changed are highlighted.

  # Nothing to do when target already equals v2+subs (already up to date).
  if diff -q "$tgt_abs" "$theirs" > /dev/null 2>&1; then
    rm -f "$base" "$theirs"
    return 0
  fi
  # Skip when v1→v2 diff is whitespace/encoding only after neutralisation.
  if diff -w -q "$tgt_abs" "$theirs" > /dev/null 2>&1; then
    rm -f "$base" "$theirs"
    return 0
  fi
  ...
  # stage1 = v1+subs (BASE), stage2 = target (LOCAL), stage3 = v2+subs (REMOTE).
  local ours_hash stage3_hash base1_hash
  ours_hash=$(git   -C "$tgt_dir" hash-object -w "$tgt_abs")
  stage3_hash=$(git -C "$tgt_dir" hash-object -w "$theirs")
