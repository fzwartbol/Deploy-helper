...
#   different value) follow one global policy, set by the optional "resolution"
#   block in repos.json:
#     "default": manual (leave <<< markers, default) | ours (keep target) | theirs
#   Non-conflicting source changes are always applied regardless.  When markers
...
# Extract the numeric PR id (first top-level "id":<n>) from a PR response JSON
_pr_id() {
  printf '%s' "$1" | tr -d '\n\r' \
    | grep -o '"id":[[:space:]]*[0-9]\+' | head -1 \
    | grep -o '[0-9]\+'
}
...
three_way_merge_file() {
  ...
  case "${RES_DEFAULT:-manual}" in
    ours)   _mf_flag="--ours" ;;
    theirs) _mf_flag="--theirs" ;;
  esac

  local rc=0
  git merge-file $_mf_flag \
    -L "target (ours)" \
    -L "base (${FROM_REF})" \
    -L "source (${TO_REF})" \
...
_bb_add_pr_task() {
  ...
  [[ -z "$api_user" || -z "$api_token" || -z "$pr_id" ]] && return 0
  local payload; payload=$(printf '{"content":{"raw":"%s"}}' "$(_json_str "$text")")
  local resp code
  resp=$(curl -s -w "\n%{http_code}" -X POST \
    -u "${api_user}:${api_token}" \
    -H "Content-Type: application/json" \
    "https://api.bitbucket.org/2.0/repositories/${repo_slug}/pullrequests/${pr_id}/tasks" \
    -d "$payload")
  code=$(tail -n1 <<< "$resp")
  if [[ "$code" == "201" ]]; then
    log_warn "  added blocking PR task (resolve conflicts before merge)"
  else
    log_warn "  could not add PR task (HTTP $code) — resolve conflicts manually before merge"
  fi
}

create_bitbucket_pr() {
  local repo="$1" branch="$2" title="$3" body="$4" conflict_task="${5:-}"
...
# Conflict-resolution policy (single global knob): manual|ours|theirs.
